...
re2.Bottom = 143
    ...
    If FULLSCREEN Then
        lWindowHandle = Render.hWnd
    Else
        lWindowHandle = Form1.Picture1.hWnd
    End If
    Set DDraw = New clsDirectDraw7
    If FULLSCREEN Then
    If Not DDraw.Startup(lWindowHandle, 1024, 768, _
        32, FULLSCREEN) Then
        MsgBox "Error creating DirectDraw primary display surface"
    End If
    Else
    If Not DDraw.Startup(lWindowHandle, 160, 144, _
        32, FULLSCREEN) Then
        MsgBox "Error creating DirectDraw primary display surface"
    End If
    End If
        'create back buffers
    Set scrb = New clsDDSurface7
    If Not scrb.Create(DDraw.ddObj, 160, _
        144) Then
        MsgBox "Error creating back buffer surface"
    End If
initCol
scrb.BltColorFill colid(0, 0)
Form1.Picture1.Visible = Not FULLSCREEN
DDraw.ddcls RGB(256, 256, 256)
